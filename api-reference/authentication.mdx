...
description: "Learn how to authenticate with the Subr API using API keys and bearer tokens."
---

The Subr API uses multiple authentication methods to ensure secure access to your data. Choose the method that best fits your use case and security requirements.

## Authentication Methods

### API Key Authentication

The simplest way to authenticate with the Subr API is using an API key passed in the request header.
...
**Example request**:
```bash title="API Key in header"
curl -X GET "https://api.subr.com/v1/pets" \
...
const response = await fetch('https://api.subr.com/v1/pets', {
  method: 'GET',
  headers: {
...
func main() {
    client := &http.Client{}
    req, _ := http.NewRequest("GET", "https://api.subr.com/v1/pets", nil)
...
```bash
curl -X POST "https://api.subr.com/v1/auth/login" \
  -H "Content-Type: application/json" \
  -d '{
    "username": "john.doe",
...
```javascript
const loginResponse = await fetch('https://api.subr.com/v1/auth/login', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
response = requests.post(
    'https://api.subr.com/v1/auth/login',
    json=login_data
)
...
class PetStoreAPI {
  ...
  async authenticate() {
    const response = await fetch('https://api.subr.com/v1/auth/login', {
      method: 'POST',
  ...
  async apiRequest(endpoint, options = {}) {
    ...
    const response = await fetch(`https://api.subr.com/v1${endpoint}`, {
      ...options,
      headers: {
    ...
    if (response.status === 401) {
      ...
      return fetch(`https://api.subr.com/v1${endpoint}`, {
        ...options,
        headers: {
...
1. **Log in to your Pet Store dashboard** at [dashboard.subr.com](https://dashboard.subr.com)
...
PETSTORE_BASE_URL=https://api.subr.com/v1
```
...
curl -X GET "https://api.subr.com/v1/users" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -w "Status: %{http_code}\n"
...
<Callout kind="success">
  **Authentication setup complete!** You're now ready to start making authenticated requests to the Subr API. Check out our [Getting Started guide](getting-started) for your first API calls.
</Callout>
...
- **[Subr API Reference](/api-reference/petstore-openapi.yaml)** - Explore all available endpoints
